...
直接插入排序在随机序列上表现不佳, 其需要进行 $\mathcal{O}(n^2)$ 次比较操作. 但是在几乎有序序列上表现尚可, 因为其可以滑过大部分已排序子串而不进行交换和重复的比较. 该算法的最差情况出现于完全逆序输入, 此时其需要进行 $(n^2 - n) / 2$ 次比较.

直接选择排序需要两两比较所有元素, 在每一趟中寻找未排序子串中给定序下的最小值. 因此, 无论输入分布如何, 均需要进行 $n^2$ 次比较操作.
